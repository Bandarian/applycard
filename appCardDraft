...
function animateStepFour(stepFour) {
  ...
  calc.forEach((btn, i) => {
    const individualTl = gsap.timeline()
      .to(btn, { scale: 0.9 })
...
function animateStepFive(stepFive) {
    ...
    gsap.set(".scales-left", { y: 0 });
    gsap.set(".scales-right", { y: 0 });
    ...
    stepFiveTl
      .to(".scales-lever", { rotationZ: -10, duration: 0.75, ease: "power1.in" })
      .to(".scale-left", { y: scalesLeftDistance, duration: 0.75, ease: "power1.in" }, "<")
      .to(".scale-right", { y: scalesRightDistance, duration: 0.75, ease: "power1.in" }, "<")
      .to(".scales-lever", { rotationZ: 5, duration: 1.5, ease: "back.out(1.7)" })
      .to(".scale-left", { y: -scalesLeftDistance / 2, duration: 1.5, ease: "back.out(1.7)" }, "<")
      .to(".scale-right", { y: -scalesRightDistance / 2, duration: 1.5, ease: "back.out(1.7)" }, "<")
      .to(".scales-lever", { rotationZ: 0, duration: 1, ease: "power1.out" }, "-=0.25")
  ...
  const blackCircles = gsap.utils.toArray(".apply-circle__inner, .apply-circle__outer");
  const gradientCircles = gsap.utils.toArray(".apply-circle__inner-gradient, .apply-circle__outer-gradient");
  gsap.set(".apply-tick", { drawSVG: 0 });
  gsap.set(".apply-circle__outer", { drawSVG: 0 });
  gsap.set(".apply-circle__overlap", { scale: 0.5, transformOrigin: "103px 103px" });
  let video = document.getElementById("confettiVid");

  const ripplePulseTl = gsap.timeline();

  ripplePulseTl
    .to(".apply-circle__inner-gradient", { attr: { r: 108 }, duration: 0.25, ease: "power1.out(1.7)" })
    .to(".apply-circle__inner-gradient", { attr: { r: 98 }, duration: 0.25, ease: "back.in(1.7)" })
 ...
 stepSixTl
    ...
    .fromTo(".apply-circle__inner", { attr: { r: 0 } }, { attr: { r: 98 }, ease: "back.out(1.7)", duration: 0.5 })
    .fromTo(".apply-tick", { drawSVG: 0 }, { drawSVG: true, ease: "power1.in", duration: 0.4 }, "-=0.1")
    .fromTo(".apply-circle__outer", { attr: { r: 98 } }, { attr: { r: 127.4 }, duration: 1.25, ease: "expo.out" }, "<")
    .fromTo(".apply-circle__overlap", { scale: 0.5, transformOrigin: "103px 103px" }, { scale: 1.5, transformOrigin: "103px 103px", duration: 1.25, ease: "expo.out" }, "<")
    .fromTo(blackCircles, { opacity: 1 }, { opacity: 0, duration: 0.5, ease: "power1.inOut" }, "-=0.5")
    .fromTo(gradientCircles, { opacity: 0, transformOrigin: "50% 50%" }, { opacity: 1, transformOrigin: "50% 50%", duration: 0.5, ease: "power1.inOut" }, "<")
    .add(ripplePulseTl);
